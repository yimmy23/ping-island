...
- If you add a Claude-compatible hook client, start in `PingIsland/Models/ClientProfile.swift` and wire any truly client-specific behavior from there before adding new ad-hoc switches elsewhere.
  ...
  - Qoder-family hook installs currently cover both `~/.qoder/settings.json` and `~/.qoderwork/settings.json`; `~/.qoder/settings.json` should track Claude Code-compatible lifecycle coverage for Qoder CLI / IDE, and app launch should refresh it when `qodercli -v` is newer than 0.2.5 while preserving unrelated JSON settings. QoderWork remains a separate hook-only profile unless its protocol converges.
