...
func installerRemovesGeneratedExtensionFolders() throws {
    ...
    let installer = IDEExtensionInstaller(homeDirectory: root)
    try installer.installTraeExtension()
    try installer.installCodeBuddyExtension()
    try installer.installQoderExtension()
    try installer.uninstallExtensions(relativeRoots: [".trae/extensions", ".codebuddy/extensions", ".qoder/extensions"])

    let traeExtensionURL = root
        .appending(path: ".trae/extensions", directoryHint: .isDirectory)
        .appending(path: "ping-island.session-focus-1.0.0", directoryHint: .isDirectory)
    ...
    #expect(!FileManager.default.fileExists(atPath: traeExtensionURL.path()))
