...
final class QwenHookCompatibilityPatchTests: XCTestCase {
    func testPatchedQwenCLISourceInjectsAnswerPayloadIntoHookHandledPaths() {
        ...
        XCTAssertEqual(
            patched?.components(separatedBy: "const hookAnswerPayload = confirmationDetails.type === \"ask_user_question\"").count,
            3
        )
        XCTAssertTrue(patched?.contains("ToolConfirmationOutcome.ProceedOnce, hookAnswerPayload") ?? false)
        XCTAssertTrue(patched?.contains("ToolConfirmationOutcome.ProceedOnce,\n                hookAnswerPayload") ?? false)
    }
